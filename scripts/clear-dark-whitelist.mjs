...
const revRef = await db.collection('revisions').add({
  ...
  note: 'pre clear-dark-whitelist backup',
  data,
})
...
const variants = data.variants.map((v) => {
  ...
  return {
    ...
    colorOptions: v.colorOptions.map((c) => {
      if (c.label !== 'Dark' || !c.applyToLayerLabels) return c
      patched++
      const { applyToLayerLabels, ...rest } = c
      return rest
    }),
  }
})
...
console.log(`whitelist cleared on ${patched} variants — Dark applies everywhere again`)
